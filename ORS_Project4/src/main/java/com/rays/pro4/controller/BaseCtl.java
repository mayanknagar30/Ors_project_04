...
public abstract class BaseCtl extends HttpServlet {
	...
	protected BaseBean populateDTO(BaseBean dto, HttpServletRequest request) {
		...
		System.out.println("basectl ki dto chali");
...
//		System.out.println(" baseCtl populateDTO parents se true mila");
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("Bctl ki service method chali");

		// Load the preloaded data required to display at HTML form
		preload(request);

		String op = DataUtility.getString(request.getParameter("operation"));
		System.out.println("Bctl servic op " + op);
		// Check if operation is not DELETE, VIEW, CANCEL, and NULL then
		// perform input data validation

		if (DataValidator.isNotNull(op) && !OP_CANCEL.equalsIgnoreCase(op) && !OP_VIEW.equalsIgnoreCase(op)
				&& !OP_DELETE.equalsIgnoreCase(op) && !OP_RESET.equalsIgnoreCase(op)) {
			System.out.println("Bctl 5 operation True hue" + "_____________" + op);
			// Check validation, If fail then send back to page with error
			// messages

			if (!validate(request)) {
				System.out.println("basectl pr 5 method ke bad if chala");
